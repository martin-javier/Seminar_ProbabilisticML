...
point will form an almost smooth curve. This structural consistency arises because all samples derive from the
shared Gaussian approximation of the weight posterior. MC-Dropout's more irregular dispersion reflects the random
neuron deactivations from different dropout masks \citep{gal2016mcdropout}.
...
out-of-distribution (OOD) scenarios are examined. Natural OOD samples with exceptionally high crime rate (\texttt{crim}) and artificially perturbed samples with amplified crime rate and reduced number of rooms.
...
For natural OOD samples, MC-Dropout shows better point-prediction accuracy with the RMSE decreasing from 0.4033 to
